...
export const useInterviewStore = defineStore(
  ...
  () => {
    // ===== 面试会话 =====
    const interviewType = ref<string | null>(null) // 当前面试类型 key
    const questions = ref<InterviewQuestion[]>([]) // 本轮题目列表
    const currentIndex = ref(0) // 当前题目下标
    const phase = ref<AnswerStatus>('idle') // idle | answering | scoring | feedback | finished
    const answers = ref<Record<string, string>>({}) // { [questionId]: userAnswer }
    const scores = ref<Record<string, InterviewScore>>({})
    const conversations = ref<Record<string, ConversationMessage[]>>({})
    ...
    function startInterview(
      ...
      const typeConfig = (interviewTypes as Record<string, any>)[typeKey]
    ...
    function loadCustomQuestions(questionsList: any[], source = ''): boolean {
    ...
    /** 提交答案，进入评分/评估阶段 */
    function submitAnswer(questionId: string, answer: string, useDeepMode = false): void {
    ...
    /** 保存 AI 评分结果（普通模式） */
    function saveScore(questionId: string, scoreData: InterviewScore): void {
    ...
    /** 处理深度评估结果（多轮追问模式） */
    function handleEvaluateResult(questionId: string, result: any): 'follow_up' | 'complete' {
    ...
    /** 重置当前题目的状态 */
    function resetCurrentQuestionState(qId: string): void {
    ...
    function nextQuestion(): void {
      ...
      if (isLastQuestion.value) {
        ...
        if (curQId) resetCurrentQuestionState(curQId)
        currentIndex.value++
        phase.value = 'answering'
      }
    }
