...
router.post('/generate', async (req: Request, res: Response) => {
  ...
  const prompt = GENERATE_PROMPT.replace('{content}', trimmedContent + truncationNote)
 ...
 */
router.post('/generate-by-role', async (req: Request, res: Response) => {
  ...
  const prompt = ROLE_PROMPT.replace('{role}', role.trim())
    .replace('{count}', String(count))
    .replace('{difficulty}', difficultyText)
  ...
  try {
    ...
    console.log(
      `[questions/generate-by-role] 成功生成 ${result.length} 道题目（岗位: ${role.trim()}）`,
    )
    res.json({ questions: result })
  } catch (err) {
