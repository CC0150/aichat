...
import type { AgentStreamEvent } from '../utils/agentLoop'
import { repairJson } from '../utils/parseJson'
import { getEmbedding } from './embedding'
import { search, addChunks, deleteByKB } from './vectorStore'
import { callAI } from './aiCompletions'
import { chunkSemantic } from './chunker'
import type {
  ToolDefinition,
  AgentEvaluateResult,
  InterviewQuestion,
...
function safeParseJson(text: string, logTag: string): any {
  try {
...
const tools: ToolDefinition[] = [
  {
    ...
    function: {
      ...
      description:
        '从知识库中搜索相关文档内容。当需要查找特定技术知识、面试题素材、或验证答案准确性时使用。',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        '根据技术主题和难度生成一道新的面试题。当需要追问更深层知识或替换当前题目时使用。',
      parameters: {
        type: 'object',
        properties: {
...
interface InterviewEvaluateParams {
  question: string
 ...
 * Agent 驱动的面试评估
 */
export async function runInterviewEvaluate({
  question,
  answerPoints,
  conversationHistory = [],
  kbId,
...
- 完成: {"action":"complete","score":1-10,"correctness":1-10,"completeness":1-10,"clarity":1-10,"feedback":"评价","improvedAnswer":"参考答案"}`
  ...
  const { text, steps }: AgentLoopResult = await agentLoop({
    ...
    messages: [
      {
        role: 'user',
        content: `请评估考生对"${question}"的回答。${kbId ? `可以先搜索知识库 ${kbId} 获取参考材料。` : ''}最后只输出 JSON。`,
      },
    ],
    maxSteps: 5,
  ...
  try {
    ...
    if (parsed.action === 'follow_up') {
      return {
        action: 'follow_up',
        followUpQuestion: parsed.followUpQuestion || '请进一步说明。',
        scoreHint: parsed.scoreHint ?? 5,
        agentSteps: steps,
      }
    }
    return {
      ...
      correctness: parsed.correctness ?? 5,
      completeness: parsed.completeness ?? 5,
      clarity: parsed.clarity ?? 5,
      feedback: parsed.feedback || '暂无评价',
...
/**
 * Agent 驱动的面试评估 —— SSE 流式版本
 *
 * 和 runInterviewEvaluate 功能相同，但通过 AsyncGenerator 逐步骤 yield 事件，
 * 前端可以实时看到 Agent 的"思考 → 搜知识库 → 评分 → 追问"全过程。
 */
export async function* runInterviewEvaluateStream({
  question,
  answerPoints,
  conversationHistory = [],
  kbId,
...
- 完成: {"action":"complete","score":1-10,"correctness":1-10,"completeness":1-10,"clarity":1-10,"feedback":"评价","improvedAnswer":"参考答案"}`
  ...
  for await (const event of agentLoopStream({
    ...
    // 解析 Agent 最终输出的 JSON，包装成 done 事件
    ...
    try {
      const parsed = safeParseJson(jsonMatch[0], 'interview/evaluate-stream')
      if (parsed.action === 'follow_up') {
        yield {
          type: 'done',
          steps: event.steps,
...
仅返回 JSON 数组。`,
    messages: [
      {
        role: 'user',
        content: `请基于知识库 ${kbId} 生成 ${count} 道面试题，难度${difficulty}。先搜索知识库了解内容。`,
      },
    ],
    maxSteps: 5,
  ...
  try {
    ...
    return {
      questions: questions.map(
        (q: any, i: number): InterviewQuestion => ({
          id: `ag-${Date.now()}-${i}`,
          type: q.type || 'concept',
...
export async function reindexKB(userId: number, kbId: string): Promise<ReindexResult> {
  ...
  for (const file of files) {
    try {
      ...
      const chunks = await chunkSemantic(content, { chunkSize: 500, overlap: 100 })
